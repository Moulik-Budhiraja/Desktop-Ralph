...
enum OXAActionError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .noSnapshot:
            "No cached query snapshot available. Run a cached query first."
        case let .unknownElementReference(reference):
            "Unknown element reference '\(reference)'. Re-run the query to refresh refs."
...
struct OXAHotkeyChord: Equatable, Sendable {
    let modifiers: [String]
    let baseKey: String
}

enum OXAScrollDirection: String, Equatable, Sendable {
    case up
    case down
    case left
    case right
...
enum OXAStatement: Equatable, Sendable {
...
struct OXAProgram: Equatable, Sendable {
