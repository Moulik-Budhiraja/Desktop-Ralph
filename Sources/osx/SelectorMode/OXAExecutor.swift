...
enum OXAExecutor {
        ...
        middleware: any ActionExecutionMiddleware = NoopActionExecutionMiddleware()) throws -> String
    {
        ...
        for (index, statement) in program.statements.enumerated() {
            let context = try self.makeExecutionContext(
                for: statement,
    ...
    private static func makeExecutionContext(
        ...
        statementIndex: Int,
        totalStatements: Int) throws -> ActionExecutionContext
    {
        let targetReferences = self.references(for: statement)
        let targets = try targetReferences.map { reference in
            _ = try self.resolveElementReference(reference)
            return ActionTargetDescriptor(
                reference: reference,
                frame: SelectorActionRefStore.frame(for: reference),
                role: SelectorActionRefStore.role(for: reference))
        }
        ...
        return ActionExecutionContext(
            ...
            targets: targets,
            snapshotAppPID: SelectorActionRefStore.snapshotAppPID)
    }

    private static func references(for statement: OXAStatement) -> [String] {
        switch statement {
        case let .sendText(_, targetRef),
             let .sendTextAsKeys(_, targetRef),
             let .sendClick(targetRef),
